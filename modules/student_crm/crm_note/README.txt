Introduction
------------
The crm_note Entities module provides a fully working yet generic example
...
Admnistration > Structure > crm_note Types

You can add crm_note entities via

Administration > Content > crm_notes

Keep in mind that you need to create some crm_note Types before you can add entities.


Using the code in your projects
-------------------------------
The way I envision using the module in my own projects is, for the time being,
searching and replacing the word "crm_note" with the actual name I want to give my
...
Your would only every really need to change the crm_note entity and crm_note type as crm_noteType
...
crm_note.admin.inc and customize the behaviour of your entity on save, delete which you
do via the crm_noteController class in crm_note.module.
...
to your table)  - just like the crm_note entity does with the checkbox - there is nothing
else you need to do. If you have added new columns you need to add support for them
in $crm_note->create but not necessarily $crm_note->save unless you are doing something specific.

3. Finally you can play around within the theming for your entity by looking into crm_note.tpl.php and
crm_note-sample-data.tpl.php
